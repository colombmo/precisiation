...
class Precisiation:
	...
	def precisiate_v2_partial(self, word, category, lvl=2):
		...
		# Using all synonyms !! What happens if we take only the best meaning? 
		# -> We can get the best by seeing the overlaps with the words from the "category"
		# -------
		### SEEMS TO BE WORKING BETTER WHEN TAKING ALL SYNONYMS! ###
		#synonyms = self.syndb.get(word)
		
		#overlaps = [self.similarity.count_overlaps(self.wordsdb[category], meaning) for meaning in synonyms]

		#best_synonyms = synonyms[np.argmax(overlaps)]		

		t = []
		#for w in best_synonyms:
		#-------------------------
		for w in self.flatten(self.syndb.get(word)):
			p = self.precisiate(w, category, lvl = lvl)
			if p != None and p != -1:
				t.append(p)
